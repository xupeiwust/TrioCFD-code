...
# Compare to ref
# Comparison is done only between seq/ref_seq and par/ref_par for now
for cas in "" "PAR_"
do
	if [ -f ${cas}${name}.lata ] ; then 
		echo Comparing ${cas}${name}.lata ref/${cas}${name}.lata
		if compare_lata ${cas}${name}.lata ref/${cas}${name}.lata 
		then
			echo "ok"
		else
