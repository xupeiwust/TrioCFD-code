...
class TrustFile:
    ...
    def _reset(self):
        self._entries = (
            {}
        )  # key = entry_name, val=col_index (or component numb for segs)
        self._taxis = TrustFile._VOID_ARR  # time axis values
        self._yvalues = TrustFile._VOID_ARR  # a np.array, col index is in _entries
        self._lastIndex = (
            {}
        )  # position of the last fetched data in the yvalues arrays. key=entry_name, vals=index
        ...
        try:
            ...
            if siz < self._lastSize:
                if update:
                    ...
                    self._reset()
                print(
                    "WARNING: file is smaller but reference file has not changed! Will reset. (%s)"
                    % self._filePath
                )
                return "reset"
    ...
    def _parseData(self, data):
        ...
        if arrData.shape[0] % self._ncols:
            raise Exception(
                "Misformatted file: the line below has an invalid numb of cols:\n%s" % a
            )
    ...
    def _populateFromHeader(self, hdr=None):
        ...
        try:
            ...
            for i in range(npoints):
                pt = [
                    float(a)
                    for a in hdr[1][2 + i * 2 * dim : 2 + (i + 1) * 2 * dim : 2]
                ]
                pts.append(pt)
