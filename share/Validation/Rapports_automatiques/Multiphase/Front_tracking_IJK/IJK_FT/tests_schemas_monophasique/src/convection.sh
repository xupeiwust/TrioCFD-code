...
for schx in QUICK CENTRE4
do
   mkdir $schx 
   cd $schx
   cp ../${jdd}.data .
   ln -s ../init.lata* .
   if [ $schx == "CENTRE4" ] ; then
      sed -i -e "s/ Quick/ Centre/" ${jdd}.data
   fi
   echo "Schema convection  :   $schx ....."
...
plot   "../QUICK/cvgx_L2_EUL.txt" u 1:3 t "Quick - EUL", \
